...
def main():
	...
	out_json = {"injson":in_json, "taskstatus":1}
        ...
        if doc["filetype"] == "csv":
		out_json["dataframe"] = []
		inFile = serverJson["pathinfo"]["htmlpath"] + "/datasets/reviewed/" + doc["filename"]
		...
		sys.exit()
            ...
            fileList = glob.glob(serverJson["pathinfo"]["htmlpath"] + "/datasets/glycanimages/*.png")
            for f in fileList[0:10]:
                recordId = f.split("/")[-1].split(".")[0]
                url = serverJson["rootinfo"]["htmlroot"] + "/datasets/glycanimages/"  + f.split("/")[-1]
                print "%s <br>" % (recordId)
                print "<img src=\"%s\"><hr><br>" % (url)
            sys.exit()
        elif doc["filetype"] in ["aln"]:
            fileList = glob.glob(serverJson["pathinfo"]["htmlpath"] + "/datasets/aln/"+species_short+"/*.aln")
            for f in fileList[0:10]:
                recordId = f.split("/")[-1].split(".")[0]
                print "%s" % (recordId)
                with open(f, 'r') as FR:
