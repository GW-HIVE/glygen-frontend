...
const ProteinDetail = (props) => {
  ...
  const recommendedGeneRows = geneNames.map(getRecommendedRows).filter((arg) => arg !== null);
  const synonymGeneRows = geneNames.map(getSynonymRows).filter((arg) => arg !== null);
  const recommendedProteinRows = proteinNames.map(getRecommendedRows).filter((arg) => arg !== null);
  const synonymProteinRows = proteinNames.map(getSynonymRows).filter((arg) => arg !== null);
