...
export default function UserTrackingBanner(props) {
    ...
    useEffect(() => { 
        ...
        if (props.userTrackingBannerState !== "none" && 
            props.userTrackingBannerState !== "display") {
            setTimeout(() => {props.setUserTrackingBannerState('none')}, 10000);
        }
  ...
  return (
    <>
    {props.userTrackingBannerState === "display" && <div className="alert gg-alert">
        ...
        <br/>
        <Button type="button" className="btn btn-default gg-btn-margin" onClick={() => {props.setUserTrackingBannerState("track")}}>Allow</Button>
        <Button type="button" className="btn btn-default gg-btn-margin" onClick={() => {props.setUserTrackingBannerState("donottrack")}}>Don't Allow</Button>
    </div>}
    {props.userTrackingBannerState === "track" && <div className="alert gg-alert">
        <span>We will log your actions to improve the user experience. You can always change this setting in <strong>My GlyGen</strong>.</span><span className="close_banner" onClick={close} style={{float: 'right'}}>&times;</span>
    </div>}
    {props.userTrackingBannerState === "donottrack" && <div className="alert gg-alert">
        <span>We will not log your actions. You can always change this setting in <strong>My GlyGen</strong>.</span><span onClick={close} className="close_banner" style={{float: 'right'}}>&times;</span>    
    </div>}
    {props.userTrackingBannerState === "browser" && <div className="alert gg-alert">
        <span>Please update your browser to the latest version in order to access all our website features.</span><span onClick={close} className="close_banner" style={{float: 'right'}}>&times;</span>    
    </div>}
    </>
  );
}
